import {
  ...
  THEO_DIALOGS, RAPH_DIALOGS,
} from './data.js';

import {
  defaultState, saveGame, loadGame,
  generateShopCards, cardPrice,
  advanceDay, addXP, xpProgress, albumCap,
  bonPapaQuote, daysUntilBonPapa,
  generatePackCards,
} from './game.js';
...
window.G = {
  async start() {
    ...
    render();
    startDialogs();
    toast(`🎁 Bienvenue ${name} ! Naïm t'a donné 10 cartes !`, 'gold');
  },
...
function renderShop() {
  ...
  S.shopCards.forEach(card => {
    ...
    div.innerHTML = `
      ...
      <div class="rarity-badge rb-${card.rarity}">${card.rarity.toUpperCase()}</div>
...
function renderBoosters() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    div.innerHTML = `
      <div class="booster-icon">${used ? '✅' : bt.emoji}</div>
      <div class="booster-label">${bt.label}</div>
      <div class="booster-price">${bt.price}€</div>`;
...
function renderAlbum() {
  ...
  S.album.forEach((card, i) => {
    ...
    div.innerHTML = `
      <div class="card-emoji">${card.emoji}</div>
      <div class="card-name">${card.name}</div>
      ${card.fromNaim ? '<div class="gift-tag">🎁 Cadeau</div>' : ''}
      <div class="card-price">${sell} €</div>
      <div class="${profit >= 0 ? 'profit-pos' : 'profit-neg'}">${profit >= 0 ? '+' : ''}${profit} €</div>`;
...
function openPackOpening(bt) {
  ...
  stage.innerHTML = `
    <div style="text-align:center">
      <div class="pack-hint">Déchire le booster !</div>
      <div class="pack-booster" id="packVisual">${bt.emoji}</div>
      <div class="pack-hint" style="margin-top:8px">Appuie pour déchirer !</div>
    </div>`;
  ...
  function advance() {
    ...
    if (phase === 'cards') {
      cardIdx++;
      if (cardIdx < packCards.length) showCard();
      else showAll();
    }
  }
  ...
  function showCard() {
    ...
    stage.innerHTML = `
      <div style="text-align:center">
        <div class="pack-counter">${cardIdx + 1} / ${packCards.length}</div>
        <div class="pack-single">
          <div class="pack-single-emoji">${c.emoji}</div>
          <div class="pack-single-name">${c.name}</div>
          <div class="rarity-badge rb-${c.rarity}">${c.rarity.toUpperCase()}</div>
        </div>
        <div class="pack-hint" style="margin-top:12px">Appuie pour continuer...</div>
      </div>`;
  ...
  function showAll() {
    stage.innerHTML = `
      <div style="text-align:center;width:100%">
        <div style="font-family:'Baloo 2',sans-serif;font-size:1.1rem;font-weight:800;color:var(--gold);margin-bottom:14px">🎉 Tes cartes !</div>
        <div class="pack-all-grid">
          ${packCards.map((c, i) => `
            <div class="pac" style="animation-delay:${i * .1}s">
              <div class="pac-emoji">${c.emoji}</div>
              <div class="pac-name">${c.name}</div>
            </div>`).join('')}
        </div>
        <button class="btn-pack-go" onclick="closePack()">✅ Partir !</button>
      </div>`;
    stage.onclick = null;
    autoSave();
  }
